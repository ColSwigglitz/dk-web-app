...
async function refreshLiveStats(){
  ...
  try {
    ...
    if(data){
      ...
      state.liveStats=Object.fromEntries(Object.entries(data.points).map(([id,p])=>[id,{...p,status:'STATS',summary:'Weekly stats received'}]));
      ...
      setText('statsStatus',`${stale?'Feed delayed · last successful update':'Updated'} ${new Date(data.fetched_at).toLocaleString()} · ${data.week_complete?'All games final — rosters revealed':'Other rosters stay private until every game is final'}`);
      setText('contestStatus',data.week_complete?'FINAL':weekLocked()?'LOCKED':'OPEN');
      setText('lockNotice',`Roster deadline: ${new Date(data.lock_at).toLocaleString()}. This first release locks the whole week at 00:00 UTC on its first game date.`);
    }else{
...
renderLeaderboard=function(){
  ...
  document.getElementById('leaderboard').innerHTML=rows.length?rows.map((r,i)=>`<div class="leader-row"><div class="leader-rank">${i+1}</div><div class="leader-name"><strong>${escapeHtml(r.display_name)}${r.user_id===state.user?.id?' · YOU':''}</strong><span>${r.selected_count===9?'Roster complete':`${r.selected_count}/9 selected`}${r.unmapped_count?' · Player mapping needed':''}</span></div><div class="leader-live">${r.week_complete?'FINAL':'PRIVATE'}</div><div class="leader-score">${r.score==null?'—':Number(r.score).toFixed(2)}</div></div>${r.week_complete&&r.lineup?`<details class="panel-note"><summary>${escapeHtml(r.display_name)} — roster and scoring breakdown</summary>${SLOT_ORDER.filter(slot=>r.lineup[slot]).map(slot=>{const p=r.lineup[slot],stat=state.statsSnapshot?.points[p.id];return `<p>${escapeHtml(slot)} · ${escapeHtml(p.name)} · ${stat?Number(stat.fantasyPoints).toFixed(2):'0.00'} pts</p><small>${escapeHtml(JSON.stringify(stat?.stats||{}))}</small>`}).join('')}</details>`:''}`).join(''):'<div class="panel-note">No scores available yet.</div>';
};
