...
const STORAGE_KEY='weeklyNFLDraftMVP-v3';
...
function playerVisual(player,size='normal'){
  ...
  const encoded=escapeAttr(JSON.stringify(urls));
  return `<div class="player-photo-wrap ${size==='small'?'player-photo-wrap-small':''}"><img class="player-photo" src="${escapeAttr(urls[0])}" data-image-sources="${encoded}" data-image-index="0" alt="${escapeAttr(player.name)}" loading="lazy"><div class="player-avatar player-photo-fallback ${size==='small'?'player-avatar-small':''}">${escapeHtml(player.position)}</div></div>`;
}
function handleImageError(img){
  let sources=[];try{sources=JSON.parse(img.dataset.imageSources||'[]')}catch(e){}
  const next=Number(img.dataset.imageIndex||0)+1;
  if(next<sources.length){img.dataset.imageIndex=String(next);img.src=sources[next];return}
  img.style.display='none';const fallback=img.nextElementSibling;if(fallback)fallback.style.display='grid';
}
...
function generatedSalary(position,rank,total){const ranges={QB:[4000,8000],RB:[4000,9000],WR:[3500,9000],TE:[3000,7000],DST:[2500,4000]};const [min,max]=ranges[position];const pct=total<=1?1:1-(rank/(total-1));const curve=Math.pow(Math.max(0,pct),0.72);return Math.max(min,Math.min(max,round100(min+(max-min)*curve)))}
function generatedProjection(position,salary){const base={QB:8,RB:5,WR:4,TE:3,DST:4}[position]||3;const factor={QB:0.0022,RB:0.00215,WR:0.0021,TE:0.002,DST:0.0014}[position]||0.002;return Math.round((base+(salary-2500)*factor)*10)/10}
...
function buildSleeperPool(raw){
  ...
  const limits={QB:40,RB:90,WR:120,TE:60,DST:32};const out=[];
...
async function syncSleeper(force=false){
  const status=document.getElementById('sleeperStatus');if(!force&&state.players.length&&Date.now()-Number(state.lastSleeperSync||0)<DAY)return;if(status)status.textContent='Refreshing free NFL player data from Sleeper…';
  try{const [playersRes,stateRes]=await Promise.all([fetch(SLEEPER_PLAYERS,{cache:'no-store'}),fetch(SLEEPER_STATE,{cache:'no-store'})]);if(!playersRes.ok)throw new Error(`Sleeper player request failed (${playersRes.status})`);const raw=await playersRes.json();const nflState=stateRes.ok?await stateRes.json():null;const pool=buildSleeperPool(raw);if(pool.length<100)throw new Error('Sleeper returned an unexpectedly small player pool.');state.players=pool;state.lastSleeperSync=Date.now();state.nflState=nflState;save();renderAll();updateWeekLabels();if(status)status.innerHTML=`<span class="success">Loaded ${pool.length} NFL players from Sleeper. Salaries generated locally. Last sync: ${new Date().toLocaleString()}.</span>`}
  catch(err){if(!state.players.length)state.players=FALLBACK_PLAYERS;renderAll();if(status)status.innerHTML=`<span class="danger">Sleeper refresh failed: ${escapeHtml(err.message)}. Using cached/fallback data.</span>`}
}
function updateWeekLabels(){const season=state.nflState?.season||'2026',week=state.nflState?.week||1;document.querySelectorAll('[data-week-label]').forEach(e=>e.textContent=`WEEK ${week}`);const hero=document.getElementById('heroWeek');if(hero)hero.textContent=`WEEK ${week} · ${season}`}
...
function renderLineup(){const wrap=document.getElementById('lineupSlots');if(!wrap)return;wrap.innerHTML=SLOT_ORDER.map(slot=>{const p=state.lineup[slot],label=slot.replace(/\d/g,'');return `<div class="lineup-slot"><div class="slot-label">${label}</div>${p?`<div class="slot-player-with-photo">${playerVisual(p,'small')}<div class="slot-player"><strong>${escapeHtml(p.name)}</strong><span>${p.position} · ${p.team}</span></div></div><div><div class="slot-salary">${money(p.salary)}</div><button class="slot-remove" data-remove="${slot}" ${state.submitted?'disabled':''}>×</button></div>`:'<div class="slot-empty">Select a player</div><div></div>'}</div>`}).join('');bindImageFallbacks(wrap);const used=salaryUsed();document.getElementById('salaryUsed').textContent=money(used);document.getElementById('salaryRemaining').textContent=money(SALARY_CAP-used);document.getElementById('projectionTotal').textContent=projected().toFixed(1);const b=document.getElementById('submitLineupBtn');b.textContent=state.submitted?'LINEUP SUBMITTED':'SUBMIT LINEUP';b.disabled=state.submitted}
function renderMyTeam(){const list=document.getElementById('myTeamList'),players=lineupPlayers();list.innerHTML=players.length?SLOT_ORDER.filter(s=>state.lineup[s]).map(slot=>{const p=state.lineup[slot],stat=state.liveStats[p.id]||{};return `<div class="team-row"><strong class="slot-label">${slot.replace(/\d/g,'')}</strong><div class="team-player-with-photo">${playerVisual(p,'small')}<div><strong>${escapeHtml(p.name)}</strong><div class="player-meta">${p.team} · ${stat.summary||'Not started'}</div></div></div><div class="game-state ${stat.status==='LIVE'?'live':''}">${stat.status||'NS'}</div><div class="live-points">${currentFantasyPoints(p).toFixed(1)}</div></div>`}).join(''):'<div class="admin-note">No players selected yet.</div>';bindImageFallbacks(list);document.getElementById('myLiveScore').textContent=players.reduce((t,p)=>t+currentFantasyPoints(p),0).toFixed(1);document.getElementById('playersLive').textContent=players.filter(p=>state.liveStats[p.id]?.status==='LIVE').length;document.getElementById('playersFinished').textContent=players.filter(p=>state.liveStats[p.id]?.status==='FINAL').length;document.getElementById('teamSalaryUsed').textContent=money(salaryUsed())}
