...
function buildSleeperPool(raw){
  ...
  Object.entries(groups).forEach(([pos,items])=>{
    ...
    chosen.forEach((p,i)=>{
      ...
      out.push({id:String(p.player_id),name:sleeperName(p,pos),position:pos,team:p.team,opp:'',salary,avg:generatedProjection(pos,salary),game:'Schedule TBC',source:'Sleeper',rank:i+1,status:p.injury_status||'',searchRank:sleeperRank(p)});
    });
  });
  return out;
}
...
function renderPlayers(){const body=document.getElementById('playerTableBody');if(!body)return;const q=state.search.toLowerCase(),selected=new Set(lineupPlayers().map(p=>p.id));const filtered=state.players.filter(p=>(state.activePosition==='ALL'||p.position===state.activePosition)&&(!q||`${p.name} ${p.team}`.toLowerCase().includes(q))).sort((a,b)=>b.salary-a.salary||a.name.localeCompare(b.name));body.innerHTML=filtered.map(p=>`<tr><td><div class="player-main"><div class="player-avatar">${p.position}</div><div><div class="player-name">${escapeHtml(p.name)}</div><div class="player-meta">${p.position} · ${p.team}${p.status?' · '+escapeHtml(p.status):''}</div></div></div></td><td><div class="matchup">${escapeHtml(p.game||'Schedule TBC')}</div></td><td><div class="avg">${Number(p.avg||0).toFixed(1)}</div></td><td><div class="salary">${money(p.salary)}</div></td><td><button class="add-btn" data-add="${escapeAttr(p.id)}" ${selected.has(p.id)||state.submitted?'disabled':''}>+</button></td></tr>`).join('')||'<tr><td colspan="5" class="avg">No players match this filter.</td></tr>'}
function renderLineup(){const wrap=document.getElementById('lineupSlots');if(!wrap)return;wrap.innerHTML=SLOT_ORDER.map(slot=>{const p=state.lineup[slot],label=slot.replace(/\d/g,'');return `<div class="lineup-slot"><div class="slot-label">${label}</div>${p?`<div class="slot-player"><strong>${escapeHtml(p.name)}</strong><span>${p.position} · ${p.team}</span></div><div><div class="slot-salary">${money(p.salary)}</div><button class="slot-remove" data-remove="${slot}" ${state.submitted?'disabled':''}>×</button></div>`:'<div class="slot-empty">Select a player</div><div></div>'}</div>`}).join('');const used=salaryUsed();document.getElementById('salaryUsed').textContent=money(used);document.getElementById('salaryRemaining').textContent=money(SALARY_CAP-used);document.getElementById('projectionTotal').textContent=projected().toFixed(1);const b=document.getElementById('submitLineupBtn');b.textContent=state.submitted?'LINEUP SUBMITTED':'SUBMIT LINEUP';b.disabled=state.submitted}
function renderMyTeam(){const list=document.getElementById('myTeamList'),players=lineupPlayers();list.innerHTML=players.length?SLOT_ORDER.filter(s=>state.lineup[s]).map(slot=>{const p=state.lineup[slot],stat=state.liveStats[p.id]||{};return `<div class="team-row"><strong class="slot-label">${slot.replace(/\d/g,'')}</strong><div><strong>${escapeHtml(p.name)}</strong><div class="player-meta">${p.team} · ${stat.summary||'Not started'}</div></div><div class="game-state ${stat.status==='LIVE'?'live':''}">${stat.status||'NS'}</div><div class="live-points">${currentFantasyPoints(p).toFixed(1)}</div></div>`}).join(''):'<div class="admin-note">No players selected yet.</div>';document.getElementById('myLiveScore').textContent=players.reduce((t,p)=>t+currentFantasyPoints(p),0).toFixed(1);document.getElementById('playersLive').textContent=players.filter(p=>state.liveStats[p.id]?.status==='LIVE').length;document.getElementById('playersFinished').textContent=players.filter(p=>state.liveStats[p.id]?.status==='FINAL').length;document.getElementById('teamSalaryUsed').textContent=money(salaryUsed())}
