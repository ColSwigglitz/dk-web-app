...
async function saveLineup(){
  ...
  const {error}=await sb.from('weekly_lineups').upsert(payload,{onConflict:'league_id,user_id,season,week'});if(error){showStatus('dbStatus',error.message,'danger');await loadCurrentLineup();renderAll();return false}else showStatus('dbStatus','Saved to shared database.','success');await loadLeagueLineups();renderLeaderboard();return true;
}
...
async function submitLineup(){if(lineupPlayers().length!==SLOT_ORDER.length){message(`Your lineup needs ${SLOT_ORDER.length-lineupPlayers().length} more players.`,'danger');return}if(salaryUsed()>SALARY_CAP){message('Lineup is over the salary cap.','danger');return}state.submitted=true;renderAll();if(await saveLineup())message('Lineup submitted and saved to the league.','success')}
async function clearLineup(){state.lineup=Object.fromEntries(SLOT_ORDER.map(s=>[s,null]));state.submitted=false;renderAll();if(await saveLineup())message('Lineup cleared.','success')}
function escapeHtml(v){return String(v??'').replace(/[&<>"']/g,s=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[s]))}
function escapeAttr(v){return escapeHtml(v)}

function bind(){
  document.getElementById('loginBtn').addEventListener('click',signIn);document.getElementById('signupBtn').addEventListener('click',signUp);document.getElementById('logoutBtn').addEventListener('click',()=>sb.auth.signOut());
  ...
  document.getElementById('positionTabs').addEventListener('click',e=>{const b=e.target.closest('[data-position]');if(!b)return;state.activePosition=b.dataset.position;document.querySelectorAll('.position-tab').forEach(x=>x.classList.toggle('active',x===b));renderPlayers()});document.getElementById('playerSearch').addEventListener('input',e=>{state.search=e.target.value;renderPlayers()});document.getElementById('playerTableBody').addEventListener('click',e=>{const b=e.target.closest('[data-add]');if(!b)return;const p=state.players.find(x=>String(x.id)===String(b.dataset.add));if(p)addPlayer(p)});document.getElementById('lineupSlots').addEventListener('click',e=>{const b=e.target.closest('[data-remove]');if(b)removeSlot(b.dataset.remove)});document.getElementById('submitLineupBtn').addEventListener('click',submitLineup);document.getElementById('clearLineupBtn').addEventListener('click',clearLineup);document.getElementById('syncSleeperBtn').addEventListener('click',()=>syncSleeper(true));
}
