...
-- Existing snapshots held every game in the week. Keep only Sunday fixtures and
-- derive the Sunday deadline; the next successful refresh will update statuses.
update public.nfl_week_stats s set
  games=(select coalesce(jsonb_agg(g),'[]'::jsonb) from jsonb_array_elements(s.games) g
    where extract(isodow from ((g->>'date')::date))=7),
  lock_at=(select min((g->>'date')::date)::timestamptz from jsonb_array_elements(s.games) g
    where extract(isodow from ((g->>'date')::date))=7),
  week_complete=false,
  fetched_at=now();
